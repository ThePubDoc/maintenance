...
function complaint(req, res) {
  ...
  const status = false;
  const maintenance_instance = new Maintenance({
    building,
    location,
    type,
    complaint,
  ...
  res.render("registered", { compid: maintenance_instance._id });
}
...
async function complaints(req, res) {
  ...
  const allComp = await Maintenance.find({ building: building, type: type });
  res.render("complaints", {
    allComp
  });
